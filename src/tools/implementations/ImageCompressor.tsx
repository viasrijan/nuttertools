  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <button onClick={compressAll} className="px-4 py-2 bg-zinc-900 text-white rounded-full text-sm">Compress</button>
        {files.some(f=>f.out) && <button onClick={downloadZip} className="px-4 py-2 border rounded-full text-sm">Download ZIP</button>}
      </div>
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
        {files.map((f,i)=><div key={i} className="border rounded-xl p-2">
          <img src={f.out||f.url} className="w-full h-28 object-cover rounded-lg"/>
