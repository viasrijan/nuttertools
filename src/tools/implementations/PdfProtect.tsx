  ...
  return (
    <div className="space-y-4 max-w-xl">
      <div className="flex gap-2">
        <button onClick={() => setAction('protect')} className={`px-4 h-9 text-sm border ${action === 'protect' ? 'bg-zinc-900 text-white' : ''}`}>Protect</button>
        <button onClick={() => setAction('unlock')} className={`px-4 h-9 text-sm border ${action === 'unlock' ? 'bg-zinc-900 text-white' : ''}`}>Unlock</button>
      </div>
      ...
      <button onClick={run} disabled={busy || !file} className="px-5 h-10 bg-zinc-900 text-white text-sm disabled:opacity-40">
