  ...
  return (
    <div className="space-y-4 max-w-2xl">
      <div className="flex gap-2">
        <button onClick={() => setType('box')} className={`px-4 h-9 text-sm border ${type === 'box' ? 'bg-zinc-900 text-white' : ''}`}>Box shadow</button>
        <button onClick={() => setType('text')} className={`px-4 h-9 text-sm border ${type === 'text' ? 'bg-zinc-900 text-white' : ''}`}>Text shadow</button>
      </div>
      {type === 'box' ? (
        <>
