  ...
  return (
    <div className="space-y-5 max-w-2xl">
      <div className="flex gap-2.5">
        ...
        <Button variant="secondary" disabled={loading} onClick={preview}>{loading ? 'Loading…' : 'Preview'}</Button>
      </div>
      ...
      {meta && (
        <>
          ...
          <div className="border p-4 max-w-sm">
            {meta.image ? <img src={meta.image} alt="" className="w-full h-48 object-cover " /> : <div className="w-full h-48 grid place-items-center bg-zinc-100 dark:bg-zinc-800 text-zinc-400 text-sm">No image</div>}
          ...
          <div className="border p-4 max-w-sm bg-black text-white  overflow-hidden">
            <div className="flex items-center gap-2 px-3 pt-3">
              <div className="w-8 h-8  bg-zinc-700 grid place-items-center text-xs">{meta.icon ? <img src={meta.icon} className="w-8 h-8 " alt="" /> : '?'}</div>
