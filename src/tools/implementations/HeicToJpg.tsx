    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 gap-3">
        {items.map((it,i)=><div key={i} className="border p-2">
          {it.url ? <><img src={it.url} className="w-full h-32 object-cover "/><a href={it.url} download={it.name} className="text-xs underline mt-1 block">{it.name} Download</a></> : <p className="text-xs text-red-500">{it.name}: {it.error}</p>}
        </div>)}
      </div>
    </div>
  )
