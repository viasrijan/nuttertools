    ...
    <div className="space-y-5">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {filtered.map(([code, name, desc]) => (
          <div key={code} className=" border border-zinc-200/80 dark:border-zinc-700/80 bg-white/50 dark:bg-zinc-900/50 p-4 transition-all duration-200 hover:shadow-[0_4px_16px_-8px_rgba(0,0,0,0.1)]">
            <div className="flex items-center gap-2">
              <span className={`px-2 py-0.5  text-[11px] font-bold ${CLASS[String(code)[0] as '1'].color}`}>{code}</span>
      ...
      <div className="flex flex-wrap gap-2.5">
        {Object.values(CLASS).map(c => (
          <span key={c.label} className={`px-2 py-1  text-[11px] font-bold ${c.color}`}>{c.label}</span>
        ))}
      </div>
    </div>
  )
