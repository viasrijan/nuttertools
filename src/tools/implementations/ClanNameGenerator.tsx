...
const ADJ = ['Shadow', 'Crimson', 'Iron', 'Void', 'Storm', 'Silent', 'Frozen', 'Golden', 'Phantom', 'Ember']
const ANIMAL = ['Foxes', 'Wolves', 'Ravens', 'Vipers', 'Panthers', 'Owls', 'Hawks', 'Serpents', 'Tigers', 'Rats']
...
export default function ClanNameGenerator() {
  ...
  const gen = () => {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const c = CLAN[Math.floor(Math.random() * CLAN.length)]
      out.push(`${a} ${b} ${c}`)
    }
    return out
  }
