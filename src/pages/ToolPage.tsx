          ...
          {Comp ? (
            <Suspense fallback={
              <div className="flex flex-col items-center justify-center gap-4 py-24">
                <span className="w-10 h-10 rounded-full border-[3px] border-zinc-200 dark:border-zinc-700 border-t-indigo-500 dark:border-t-indigo-400 animate-spin" />
                  ...
                  <ol className="space-y-3">
                    {info.howTo.map((step, i) => (
                      <li key={i} className="flex gap-3 text-[16px] font-medium text-zinc-700 dark:text-zinc-300 leading-relaxed">
                        <span className="shrink-0 w-5 h-5 rounded-full bg-gradient-to-br from-indigo-500 to-violet-600 text-white grid place-items-center text-[10px] font-bold shadow-[0_2px_8px_-2px_rgba(99,102,241,0.6)]">{i + 1}</span>
              ...
              {info.faqs && info.faqs.length > 0 && (
                <div>
                  ...
                  <div className="space-y-2.5">
                    {info.faqs.map((f, i) => (
                      <details key={i} className="group rounded-xl border border-zinc-200/80 dark:border-zinc-800 bg-white dark:bg-zinc-900/70 soft-shadow transition-colors duration-200 hover:border-zinc-300 dark:hover:border-zinc-700">
                        <summary className="cursor-pointer list-none flex items-center justify-between gap-4 px-5 py-4 text-[14.5px] font-semibold text-zinc-900 dark:text-white transition-colors group-hover:text-indigo-600 dark:group-hover:text-indigo-300">
                          {f.q}
                          <span className="shrink-0 w-6 h-6 rounded-full bg-zinc-100 dark:bg-zinc-800 grid place-items-center transition-transform duration-300 group-open:rotate-180">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
            {CATEGORIES.map((c) => (
              <Link key={c.slug} to={`/tools/${c.slug}`}
                className={`group relative overflow-hidden rounded-2xl p-4 md:p-5 ${tileGrad(c.hue)} soft-shadow transition-all duration-300 ease-out hover:-translate-y-1 hover:scale-[1.02] hover:shadow-[0_16px_40px_-12px_rgba(0,0,0,0.35)] flex flex-col items-center text-center ${c.slug === cat.slug ? 'ring-2 ring-white dark:ring-white/70' : ''}`}>
