...
import { hueFor, cardRing } from '../lib/style'
...
export default function ToolCard({ tool }: { tool: any }) {
  ...
  return (
    <Link to={`/tool/${tool.id}`}
      className={`group relative bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-4 md:p-5 transition-all duration-200 hover:shadow-xl hover:shadow-zinc-900/5 dark:hover:shadow-black/20 hover:-translate-y-0.5 ${cardRing(h)}`}>
      <span className="block w-11 h-11 transition-transform duration-200 group-hover:scale-110">
        <CutoutToolIcon id={tool.id} className="w-full h-full" />
      </span>
